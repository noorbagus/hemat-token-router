...
## Knowledge Gaps
- **182 isolated node(s):** `Langkah setelah `Reopen in Container``, `Verify`, `UX attach`, `autostart-proxy.sh script`, `UPSTREAM_API_KEY` (+177 more)
...
- **Why does `resolve_under_base()` connect `resolve_under_base` to `tool_shadow.py`, `test_proxy_inject.py`, `handle_messages_request`, `main_cli`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `ShadowStreamer` connect `ShadowStreamer` to `dispatcher.py`, `logger.py`, `forward_streaming_request`?**
...
- **What connects `Langkah setelah `Reopen in Container``, `Verify`, `UX attach` to the rest of the system?**
